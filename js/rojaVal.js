let valRoja = (row,col,rec,m1,m2,m3,m4)=>{
  if(rec === m1){
    console.log("aqui m1")
    if(matriz[row][col+1] === m2 && matriz[row+1][col] === m3 && matriz[row+1][col+1] === m4){
      objReturn = [{row:row, col:col+1}, {row:row+1, col:col}, {row:row+1, col:col+1}]; //normal
      return(1)
    }else if(matriz[row][col-1] === m2 && matriz[row+1][col] === m3 && matriz[row+1][col-1] === m4){
  ...
  if(rec === m2){
    if(matriz[row][col-1] === m1 && matriz[row+1][col] === m4 && matriz[row+1][col-1] === m3){
      objReturn = [{row:row, col:col-1}, {row:row+1, col:col}, {row:row+1, col:col-1}]; //normal
      return(1)
    }else if(matriz[row][col+1] === m1 && matriz[row+1][col] === m4 && matriz[row+1][col+1] === m3){
      objReturn = [{row:row, col:col+1}, {row:row+1, col:col}, {row:row+1, col:col+1}]; //der/izq
      return(1)
    }else if(matriz[row][col-1] === m1 && matriz[row-1][col] === m4 && matriz[row-1][col-1] === m3){
      objReturn = [{row:row, col:col-1}, {row:row-1, col:col}, {row:row-1, col:col-1}]; //abajo
